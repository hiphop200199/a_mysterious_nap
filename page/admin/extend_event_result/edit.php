...
require_once $_SERVER['DOCUMENT_ROOT'] . '/controller/admin/extend_event_result.php';
...
if(empty($info)){
  header('Location: ' . ROOT . '/page/admin/extend_event_result/list.php');
  exit;
}
?>
<div id="backend">
...
<section id="main">
      <section id="form">
        ...
        <a href="list.php" id="back">返回</a>
         <form id="extend-event-result-edit">
          <label for="">延伸事件選項</label>
          <div>  <select name="" id="extend-event-option" disabled>
            <option value="">請選擇延伸事件選項</option>
            <?php foreach($extendEventOptionList as $k=>$v):?>
              <option <?php if(htmlspecialchars($info['extend_event_option_id'])==htmlspecialchars($v['id'])){?>selected<?php }?> value="<?=$v['id']?>"><?=$v['name']?></option>
              <?php endforeach;?>
          </select><label for="" id="extend-event-option-error" class="error">必填</label></div>
          <label for="">名稱</label>
    ...
    <script src="<?=ROOT.'/js/page/admin/extend_event_result/edit.js'?>" type="module"></script>
