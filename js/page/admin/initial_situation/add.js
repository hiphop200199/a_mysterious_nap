...
const awakeDegree = document.getElementById("awake-degree");
const awakeDegreeError = document.getElementById("awake-degree-error");
...
awakeDegree.addEventListener("change", checkAwakeDegree);
...
form.addEventListener("submit", async function (e) {
  ...
  checkName();
  checkVoiceOver();
  if(!awakeDegree.value||!name.value||!voiceOver.value)return;
 
  try {
    const param = {
      awake_degree:awakeDegree.value,
      name:name.value,
      voice_over:voiceOver.value,
      image:imageFile.files[0],
      manage: "initial_situation",
      task: "create",
    };
...
function checkAwakeDegree(){
  if (!awakeDegree.value) {
    awakeDegreeError.style.display = "inline";
    return;
  }
  awakeDegreeError.style.display = "none";
}