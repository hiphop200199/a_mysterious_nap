...
const extendEventOption = document.getElementById("extend-event-option");
const extendEventOptionError = document.getElementById("extend-event-option-error");
...
extendEventOption.addEventListener('change',checkExtendEventOption);
...
form.addEventListener("submit", async function (e) {
  e.preventDefault();
  checkExtendEventOption();
  checkName();
  checkVoiceOver();

  if (!extendEventOption.value||!name.value||!voiceOver.value) return;
  
  try {
    const param = {
      extend_event_option_id:extendEventOption.value,
      name: name.value,
...
function checkExtendEventOption() {
  if (!extendEventOption.value) {
    extendEventOptionError.style.display = "inline";
    return;
  }
  extendEventOptionError.style.display = "none";
}
