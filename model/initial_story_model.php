...
class Initial_story_model
{
    ...
    {
        $sql = 'SELECT * FROM initial_story WHERE id = ? ';
        $stmt =  $this->db->conn->prepare($sql);
        $stmt->execute([$id]);
    ...
    {
        $sql = 'INSERT INTO initial_story VALUES( ?,?,?,?,? ) ';
        $stmt =  $this->db->conn->prepare($sql);
    ...
    {
        $sql = 'UPDATE initial_story SET name = ?,voice_over = ?,update_time = ? WHERE id = ?';
        $stmt =  $this->db->conn->prepare($sql);
    ...
    {
        $sql = 'DELETE FROM initial_story WHERE id = ? ';
        $stmt =  $this->db->conn->prepare($sql);
        $stmt->execute([$id]);
        if ($stmt->rowCount() == 1) {
            $result = SUCCESS;
