...
class Extend_event_result_model
{
    private $db;
    ...
    {
        $sql = 'SELECT * FROM  extend_event_result';
    ...
    {
        $sql = 'SELECT * FROM extend_event_result WHERE id = ? ';
        $stmt =  $this->db->conn->prepare($sql);
        $stmt->execute([$id]);
    ...
    public function create($extendEventOptionId,$name, $voiceOver)
    {
        $sql = 'INSERT INTO extend_event_result VALUES( ?,?,?,?,?,? ) ';
        $stmt =  $this->db->conn->prepare($sql);
        $stmt->execute([null,$extendEventOptionId, $name, $voiceOver, time(), time()]);
        if ($stmt->rowCount() == 1) {
            $result = SUCCESS;
            return $result;
        }
    ...
    {
        $sql = 'UPDATE extend_event_result SET name = ?,voice_over = ?,update_time = ? WHERE id = ?';
        $stmt =  $this->db->conn->prepare($sql);
    ...
    {
        $sql = 'DELETE FROM extend_event_result WHERE id = ? ';
        $stmt =  $this->db->conn->prepare($sql);
        $stmt->execute([$id]);
        if ($stmt->rowCount() == 1) {
            $result = SUCCESS;
