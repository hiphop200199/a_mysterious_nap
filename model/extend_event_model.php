...
class Extend_event_model
{
    ...
    public function create($initialSituationId, $name, $voiceOver, $imageSourceString)
    {
        $sql = 'INSERT INTO extend_event VALUES( ?,?,?,?,?,?,? ) ';
        $stmt =  $this->db->conn->prepare($sql);
        $stmt->execute([null,$initialSituationId, $name, $voiceOver, $imageSourceString, time(), time()]);
        if ($stmt->rowCount() == 1) {
            $result = SUCCESS;
            return $result;
        }
    ...
    public function edit($id, $name, $voiceOver, $imageSourceString)
    {
        $sql = 'UPDATE extend_event SET name = ?,voice_over = ?,image = ?,update_time = ? WHERE id = ?';
        $stmt =  $this->db->conn->prepare($sql);
        $stmt->execute([$name,$voiceOver, $imageSourceString, time(), $id]);
        if ($stmt->rowCount() == 1) {
            $result = SUCCESS;
            return $result;
        }
